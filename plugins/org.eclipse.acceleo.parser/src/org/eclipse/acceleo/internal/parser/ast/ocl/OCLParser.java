...
public class OCLParser {
	...
	private OCLExpression createAcceleoInvocation(EObject eObject,
		...
		if (eObject instanceof OperationCallExp) {
			...
			if (eCall.getReferredOperation() != null && eCall.getReferredOperation().getEAnnotation(
	...
	private boolean receiverIsArgument(OperationCallExp eCall, ModuleElement astModuleElement,
		...
		if (eCall.getSource() != null) {
			if (eCall.getSource() instanceof VariableExp && ((VariableExp)eCall.getSource())
					.getReferredVariable() != null) {
	...
	private boolean handleRecursiveCall(OperationCallExp eCall, ModuleElement astModuleElement,
		...
		if (eContainer instanceof org.eclipse.acceleo.parser.cst.Template) {
			...
			if (cstTemplate.getStartPosition() == astModuleElement.getStartPosition() && cstTemplate
					.getEndPosition() == astModuleElement.getEndPosition()) {
			...
			if (cstQuery.getStartPosition() == astModuleElement.getStartPosition() && cstQuery
					.getEndPosition() == astModuleElement.getEndPosition()) {
			...
			if (cstMacro.getStartPosition() == astModuleElement.getStartPosition() && cstMacro
					.getEndPosition() == astModuleElement.getEndPosition()) {
	...
	private void shiftOCLExpressionPositions(int posBegin, EObject eContent) {
		...
		if (eContent instanceof CallingASTNode && ((CallingASTNode)eContent)
				.getPropertyStartPosition() >= 0) {
			((CallingASTNode)eContent).setPropertyStartPosition(posBegin + ((CallingASTNode)eContent)
					.getPropertyStartPosition());
			((CallingASTNode)eContent).setPropertyEndPosition(posBegin + ((CallingASTNode)eContent)
					.getPropertyEndPosition());
		}
		if (eContent instanceof TypedASTNode && ((TypedASTNode)eContent).getTypeStartPosition() >= 0) {
			((TypedASTNode)eContent).setTypeStartPosition(posBegin + ((TypedASTNode)eContent)
					.getTypeStartPosition());
			((TypedASTNode)eContent).setTypeEndPosition(posBegin + ((TypedASTNode)eContent)
					.getTypeEndPosition());
		}
	}
	...
	private boolean equals(EOperation o1, EOperation o2) {
		...
		if (o1 == o2) {
			...
			if (o1.getName() != null && o1.getName().equals(o2.getName()) && o1.getEParameters().size() == o2
					.getEParameters().size()) {
				...
				while (params1.hasNext() && params2.hasNext()) {
					...
					if (param1.getEType() != null && param2.getEType() != null && param1.getEType()
							.getName() != null && !param1.getEType().getName().equals(param2.getEType()
									.getName())) {
						return false;
					}
				}
