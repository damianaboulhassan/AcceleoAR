...
public class CST2ASTConverterWithResolver extends CST2ASTConverter {
			...
			while (iInputIt.hasNext()) {
				...
				if (found) {
					logWarning(AcceleoParserMessages.getString(
							"CST2ASTConverterWithResolver.ModuleAlreadyUsesMetaModels"), iNext //$NON-NLS-1$
	...
	private boolean isRecursiveExtends(org.eclipse.acceleo.model.mtl.Module module,
		...
		for (org.eclipse.acceleo.model.mtl.Module extended : extendedModule.getExtends()) {
			...
			for (org.eclipse.acceleo.model.mtl.Module moduleReached : this.modulesReached) {
				if (moduleReached == extended || (EcoreUtil.getURI(moduleReached) != null && (EcoreUtil
						.getURI(moduleReached).equals(EcoreUtil.getURI(extended))))) {
	...
	private boolean isRecursiveImports(org.eclipse.acceleo.model.mtl.Module module,
		...
		for (org.eclipse.acceleo.model.mtl.Module imported : importedModule.getImports()) {
			boolean alreadyContained = false;
			for (org.eclipse.acceleo.model.mtl.Module moduleReached : this.modulesReached) {
				if (moduleReached == imported || (EcoreUtil.getURI(moduleReached) != null && (EcoreUtil
						.getURI(moduleReached).equals(EcoreUtil.getURI(imported))))) {
					alreadyContained = true;
					break;
				}
			}
	...
	private org.eclipse.acceleo.model.mtl.Module getModule(Resource oResource, String fullModuleName) {
		if (oResource != null && fullModuleName != null) {
			...
			while (itOtherResources.hasNext()) {
				...
				if (otherResource.getContents().size() > 0 && otherResource.getContents().get(
						0) instanceof org.eclipse.acceleo.model.mtl.Module) {
					org.eclipse.acceleo.model.mtl.Module otherModule = (org.eclipse.acceleo.model.mtl.Module)otherResource
							.getContents().get(0);
					if (fullModuleName.equals(otherModule.getNsURI()) || fullModuleName.equals(otherModule
							.getName())) {
						return otherModule;
					}
				}
	...
	private void transformStepResolve(org.eclipse.acceleo.parser.cst.Template iTemplate) {
		...
		if (iTemplate != null && oTemplate != null) {
			...
			while (iOverridesIt.hasNext()) {
				...
				List<org.eclipse.acceleo.model.mtl.Template> oOverrides = getExtendedTemplatesNamed(getModule(
						oTemplate), ioNext.getName(), paramTypes);
				if (oOverrides.size() == 0) {
					logProblem(AcceleoParserMessages.getString("CST2ASTConverterWithResolver.MissingTemplate", //$NON-NLS-1$
					...
					for (org.eclipse.acceleo.model.mtl.Template template : oOverrides) {
						if (template.eContainer() instanceof org.eclipse.acceleo.model.mtl.Module && oTemplate
								.eContainer() instanceof org.eclipse.acceleo.model.mtl.Module && (EcoreUtil
										.getURI(template.eContainer()).equals(EcoreUtil.getURI(oTemplate
												.eContainer())))) {
							// The overridden template is in the same module as the current template
							logWarning(AcceleoParserMessages.getString(
									"CST2ASTConverterWithResolver.OverrideTemplateInSameModule"), //$NON-NLS-1$
	...
	private void checkSameValueInOverrides(org.eclipse.acceleo.model.mtl.Template oTemplate,
		...
		if (oTemplate.getOverrides() != null && oTemplate.getOverrides().contains(template)) {
			logWarning(AcceleoParserMessages.getString("CST2ASTConverterWithResolver.TemplateAlreadyOverride", //$NON-NLS-1$
					template.getName()), posBegin, posEnd);
		}
	}
	...
	private List<org.eclipse.acceleo.model.mtl.Template> getProtectedTemplatesNamed(
		...
		while (itObjects.hasNext()) {
			...
			if (eObject instanceof org.eclipse.acceleo.model.mtl.Template) {
				...
				if (checksName(name, otherTemplate.getName(), oModule.getNsURI()) && paramTypes
						.size() == otherTemplate.getParameter().size() && otherTemplate.getVisibility()
								.getValue() >= VisibilityKind.PROTECTED_VALUE) {
					...
					while (parameterMatches && itParameters.hasNext()) {
						...
						if (eParameterType instanceof EClass && type instanceof EClass) {
							parameterMatches = eParameterType == type || isSubTypeOf((EClass)eParameterType,
									(EClass)type);
						} else {
							parameterMatches = eParameterType == type || (eParameterType != null
									&& eParameterType.equals(type));
							// For OCL collections
							boolean match = eParameterType != null && eParameterType.eClass() != null
									&& EcoreUtil.getURI(eParameterType.eClass()) != null && EcoreUtil.getURI(
											eParameterType.eClass()).equals(EcoreUtil.getURI(type.eClass()));
	...
	private void transformStepResolveAddVariable(org.eclipse.acceleo.parser.cst.Variable iVariable) {
		...
		if (iVariable != null && oVariable != null) {
			...
			org.eclipse.ocl.ecore.OCLExpression oInitExpression = factory.getOrCreateOCLExpression(
					iInitExpression);
	...
	private synchronized void transformStepResolve(
		...
		org.eclipse.ocl.ecore.OCLExpression oOCLExpression = factory.getOrCreateOCLExpression(
				iModelExpression);
		if (iModelExpression != null && oOCLExpression != null) {
			...
			if (oOCLExpression instanceof OperationCallExp && ((OperationCallExp)oOCLExpression)
					.getOperationCode() == org.eclipse.ocl.utilities.PredefinedType.EQUAL) {
				...
				if ((sourceIsCollection && !argumentIsCollection) || (argumentIsCollection
						&& !sourceIsCollection)) {
					logWarning(AcceleoParserMessages.getString(
			...
			if (oOCLExpression instanceof OperationCallExp && ((OperationCallExp)oOCLExpression)
					.getReferredOperation() != null && "invoke".equals(((OperationCallExp)oOCLExpression) //$NON-NLS-1$
			...
			if (oOCLExpression instanceof TemplateInvocation) {
				...
				if (templateInvocation.getDefinition() != null && templateInvocation.getDefinition()
						.isDeprecated()) {
					logWarning(AcceleoParserMessages.getString(DEPRECATED_TEMPLATE_MESSAGE, templateInvocation
							.getDefinition().getName()), templateInvocation.getStartPosition(),
							templateInvocation.getEndPosition());
				}
			} else if (oOCLExpression instanceof QueryInvocation) {
				QueryInvocation queryInvocation = (QueryInvocation)oOCLExpression;
				if (queryInvocation.getDefinition() != null && queryInvocation.getDefinition()
						.isDeprecated()) {
				...
				if (macroInvocation.getDefinition() != null && macroInvocation.getDefinition()
						.isDeprecated()) {
			...
			} else if (oOCLExpression instanceof BooleanLiteralExp && oOCLExpression
					.eContainer() instanceof ExpressionInOCL && oOCLExpression.eContainer()
							.eContainer() instanceof Constraint) {
				// If we are here, there is a problem
				this.logProblem(AcceleoParserMessages.getString(
						"CST2ASTConverterWithResolver.InvalidModelExpression"), iModelExpression //$NON-NLS-1$
	...
	private void detectServiceInQueryReturningString(OperationCallExp operationCallExp) {
		if (operationCallExp.eContainer() instanceof Query && ((Query)operationCallExp.eContainer())
				.getParameter().size() > 0) {
			...
			if (query.getType() != null && variable.getType() != null) {
				if (String.class.equals(query.getType().getInstanceClass()) && String.class.equals(variable
						.getType().getInstanceClass())) {
					this.logWarning(AcceleoParserMessages.getString(
							"CST2ASTConverterWithResolver.ServiceInQueryReturningAString"), query //$NON-NLS-1$
	...
	private void detectServiceInTemplate(OperationCallExp operationCallExp) {
		...
		if (container instanceof org.eclipse.acceleo.model.mtl.Template) {
			this.logWarning(AcceleoParserMessages.getString("CST2ASTConverterWithResolver.ServiceInTemplate"), //$NON-NLS-1$
	 ...
	 *            is the input object of type 'org.eclipse.acceleo.parser.cst.InitSection'
	 */
	private void transformStepResolveRemoveVariables(
	...
	private void transformStepResolve(org.eclipse.acceleo.parser.cst.ProtectedAreaBlock iProtectedAreaBlock) {
		...
		if (iProtectedAreaBlock != null && oProtectedAreaBlock != null) {
			...
			if (iProtectedAreaBlock.getMarker() != null && iProtectedAreaBlock.getMarker()
					.getBody() != null) {
				String body = iProtectedAreaBlock.getMarker().getBody();
				if ("".equals(body) || "''".equals(body)) { //$NON-NLS-1$ //$NON-NLS-2$
					this.logProblem(AcceleoParserMessages.getString(
							"CST2ASTConverterWithResolver.ProtectedAreaMissingIdentifier"), //$NON-NLS-1$
	...
	private void parseWhitespaceAfterProtectedArea(AcceleoSourceBuffer buffer, int startPosition) {
		...
		if (indexOfLineDelimiter != strBuffer.length()) {
			...
			if (startNonWhiteSpace != -1 && endNonWhiteSpace != -1) {
				this.logProblem(AcceleoParserMessages.getString(
						"CST2ASTConverterWithResolver.TextAfterProtectedAreaMarker"), //$NON-NLS-1$
	...
	private void checkNameConflict(final org.eclipse.acceleo.parser.cst.ProtectedAreaBlock iProtectedArea) {
		...
		if (!conflictList.isEmpty()) {
			for (ProtectedAreaBlock protectedAreaBlock : conflictList) {
				this.logWarning(AcceleoParserMessages.getString(
						"CST2ASTConverterWithResolver.ProtectedAreaConflict"), protectedAreaBlock //$NON-NLS-1$
								.getMarker().getStartPosition(), protectedAreaBlock.getMarker()
										.getEndPosition());
			}
		}
	}
	 ...
	 */
	private EObject getScopeContainer(
			final org.eclipse.acceleo.parser.cst.ProtectedAreaBlock iProtectedArea) {
	...
	private boolean compatibleVariableTypeFor(org.eclipse.ocl.ecore.Variable oForVariable,
		...
		if (initType != null && initType.getInstanceClass() != null && variableType
	...
	private void transformStepResolve(org.eclipse.acceleo.parser.cst.IfBlock iIfBlock) {
		...
		if (iIfBlock != null && oIfBlock != null) {
			...
			if (oIfExpr != null) {
				...
				if (oIfExpr.getType() != null && !(oIfExpr.getType() == getOCL().getOCLEnvironment()
	...
	private void transformStepResolve(org.eclipse.acceleo.parser.cst.TraceBlock iTraceBlock) {
		...
		if (iTraceBlock != null && oTraceBlock != null) {
			...
			org.eclipse.ocl.ecore.OCLExpression oModelElement = factory.getOrCreateOCLExpression(
					iModelElement);
	...
	private void transformStepResolve(org.eclipse.acceleo.parser.cst.Query iQuery) {
		...
		if (iQuery != null && oQuery != null) {
			...
			try {
				...
				org.eclipse.ocl.ecore.OCLExpression oExpression = factory.getOrCreateOCLExpression(
						iExpression);
				if (oExpression != null) {
			...
			if (oQuery.getType() != null && oQuery.getExpression() != null && oQuery.getExpression()
					.getType() != null) {
				...
				if (queryType.getInstanceClass() != null && expressionType.getInstanceClass() != null
						&& !queryType.getInstanceClass().isAssignableFrom(expressionType
								.getInstanceClass())) {
			...
			while (iBodyIt.hasNext()) {
				...
				if (isValidRegion(iNext)) {
					if (iNext instanceof org.eclipse.acceleo.parser.cst.Template) {
						...
						logProblem(IAcceleoParserProblemsConstants.SYNTAX_TEXT_NOT_VALID + ' ' + iNext
								.getClass().getName(), iNext.getStartPosition(), iNext.getEndPosition());
					}
				}
			}
		}
