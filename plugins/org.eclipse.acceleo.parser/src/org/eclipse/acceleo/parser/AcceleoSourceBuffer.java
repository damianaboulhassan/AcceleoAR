...
import org.eclipse.acceleo.common.AcceleoMetamodelsRegistry;
...
public class AcceleoSourceBuffer implements IASTProvider, IDocumentationProvider {
	...
	/**
	 * The acceleo project which contains the files to be parsed.
	 */
	protected AcceleoProject accProject;
	...
	/**
	 * Sets the acceleo project.
	 * 
	 * @param project
	 */
	public void setAcceleoProject(AcceleoProject project) {
		accProject = project;
	}

	/**
	 * Getter of the acceleo project.
	 * 
	 * @return
	 */
	public AcceleoProject getAcceleoProject() {
		return accProject;
	}
	...
	public void createAST(Resource resource) {
		...
		if (resource.getContents().size() > 0 && resource.getContents().get(
		...
		if (accProject != null && accProject.getProjectRoot() != null) {
			for (IMetamodelHandler handler : AcceleoMetamodelsRegistry.INSTANCE
					.getAllRegisteredMetamodelHandlers()) {
				for (EPackage pkg : handler.getMetamodels(accProject.getProjectRoot())) {
					astCreator.getOCL().addMetamodel(pkg);
				}
				for (String error : handler.getErrorLogs()) {
					logProblem(error, 0, 0);
				}
			}
		}
	}
