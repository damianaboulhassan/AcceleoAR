...
public class AcceleoSourceContent {
	...
	public void init(StringBuffer text, IFile aFile) {
		...
		if (aFile != null && aFile.getProject() != null) {
			source.setAcceleoProject(new org.eclipse.acceleo.internal.parser.compiler.AcceleoProject(aFile
					.getProject().getLocation().makeAbsolute().toFile()));
		}
	...
	public CSTNode updateCST(int posBegin, int posEnd, String newText) {
		...
		if (current != null && posBegin == posEnd && posBegin == current.getStartPosition()) {
			if (!(current instanceof TextExpression || current instanceof Variable
					|| current instanceof ModelExpression)) {
				current = (CSTNode)current.eContainer();
			}
		}
		while (current != null) {
			EObject modified = null;
			if (current instanceof ModuleElement && (((ModuleElement)current)
					.eContainingFeature() == CstPackage.eINSTANCE.getModule_OwnedModuleElement())) {
			...
			} else if (current instanceof ModuleElement && (((ModuleElement)current)
					.eContainingFeature() == CstPackage.eINSTANCE.getModule_Documentation())) {
	...
	private ModelExpression replaceModelExpression(ModelExpression oldExpression) {
		...
		cstParser.getPBlock().parseExpressionHeader(oldExpression.getStartPosition(), oldExpression
				.getEndPosition(), oldExpression);
	...
	private Collection<Choice> order(List<Choice> choices, OCLParser oclParser) {
		...
		while (choicesIt.hasNext()) {
			...
			if (choice.getElement() instanceof EOperation) {
				...
				org.eclipse.acceleo.model.mtl.ModuleElement eModuleElement = oclParser.getModuleElement(
						eOperation);
	...
	protected void loadImportsDependencies(org.eclipse.acceleo.model.mtl.Module vAST,
		...
		for (Iterator<URI> itDependenciesURIs = dependenciesURIs.iterator(); itDependenciesURIs.hasNext();) {
			...
			while (values.hasNext()) {
				...
				if (moduleImportsValue != null && moduleImportsValue.getName() != null
						&& (IAcceleoConstants.NAMESPACE_SEPARATOR + moduleImportsValue.getName()).endsWith(
								IAcceleoConstants.NAMESPACE_SEPARATOR + oName)) {
					org.eclipse.acceleo.model.mtl.Module otherModule = getModule(vAST.eResource()
							.getResourceSet(), oURI);
					if (otherModule != null && (moduleImportsValue.getName().equals(otherModule.getNsURI())
							|| moduleImportsValue.getName().equals(otherModule.getName())) && !vAST
									.getImports().contains(otherModule)) {
	...
	protected void loadExtendsDependencies(org.eclipse.acceleo.model.mtl.Module vAST,
		...
		for (Iterator<URI> itDependenciesURIs = dependenciesURIs.iterator(); itDependenciesURIs.hasNext();) {
			...
			while (values.hasNext()) {
				...
				if (moduleExtendsValue != null && moduleExtendsValue.getName() != null
						&& (IAcceleoConstants.NAMESPACE_SEPARATOR + moduleExtendsValue.getName()).endsWith(
								IAcceleoConstants.NAMESPACE_SEPARATOR + oName)) {
					org.eclipse.acceleo.model.mtl.Module otherModule = getModule(vAST.eResource()
							.getResourceSet(), oURI);
					if (otherModule != null && (moduleExtendsValue.getName().equals(otherModule.getNsURI())
							|| moduleExtendsValue.getName().equals(otherModule.getName())) && !vAST
									.getExtends().contains(otherModule)) {
	...
	private org.eclipse.acceleo.model.mtl.Module getModule(ResourceSet resourceSet, URI uri) {
		try {
			...
			AcceleoUIActivator.getDefault().getLog().log(new Status(IStatus.ERROR,
					AcceleoUIActivator.PLUGIN_ID, e.getMessage(), e));
	...
	public boolean isInModuleHeader(int posBegin, int posEnd, boolean inAST) {
		...
		int endHeaderModule = this.source.getBuffer().indexOf(IAcceleoConstants.DEFAULT_END_BODY_CHAR
				+ IAcceleoConstants.DEFAULT_END, beginHeaderModule);
	...
	protected int compareOperations(Choice arg0, Choice arg1) {
		...
		boolean arg0IsTemplate = arg0 instanceof AcceleoCompletionChoice && ((AcceleoCompletionChoice)arg0)
				.getAcceleoElement() instanceof org.eclipse.acceleo.model.mtl.Template;
		boolean arg1IsTemplate = arg1 instanceof AcceleoCompletionChoice && ((AcceleoCompletionChoice)arg1)
				.getAcceleoElement() instanceof org.eclipse.acceleo.model.mtl.Template;
		boolean arg0IsQuery = arg0 instanceof AcceleoCompletionChoice && ((AcceleoCompletionChoice)arg0)
				.getAcceleoElement() instanceof org.eclipse.acceleo.model.mtl.Query;
		boolean arg1IsQuery = arg1 instanceof AcceleoCompletionChoice && ((AcceleoCompletionChoice)arg1)
				.getAcceleoElement() instanceof org.eclipse.acceleo.model.mtl.Query;
